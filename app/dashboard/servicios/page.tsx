...
export default function ServiciosPage() {
  ...
  useEffect(() => {
    const fetchAndCheck = async () => {
      ...
      if (org) {
        ...
        // Solo redirigir si NO es activo y ya tiene servicios
  ...
  const handleSyncWithAI = async () => {
    ...
    try {
      ...
      if (isManagementMode) alert("✨ VORA sincronizada.");
      ...
      <header className="mb-12 text-center flex flex-col items-center max-w-4xl w-full">
        <div className="flex items-center gap-2 mb-3">
         ...
         <h1 className="text-4xl font-black text-slate-950 tracking-tight">
            Mis <span className="text-emerald-600 italic">Servicios</span>
          </h1>
        ...
        <div className="mt-8 flex items-center gap-4 bg-white/60 backdrop-blur-sm px-6 py-3 rounded-full border border-slate-100 shadow-sm">
            ...
            <span className={`text-[9px] font-black uppercase ${showPricesOnChat ? 'text-emerald-600' : 'text-slate-400'}`}>{showPricesOnChat ? 'Activado' : 'Oculto'}</span>
        </div>
      </header>
        ...
        {services.map(service => (
          <div key={service.id} className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm hover:shadow-2xl hover:border-rose-100 transition-all relative flex flex-col justify-between group">
            ...
            <div className="grid grid-cols-3 gap-4 pt-8 border-t border-slate-50">
              <div className="flex flex-col"><span className="text-[9px] text-slate-400 uppercase font-black tracking-widest mb-1">Precio</span><span className="text-xl font-black text-rose-600 italic">{orgData?.currency_symbol}{service.price}</span></div>
              <div className="flex flex-col border-l border-slate-50 pl-4"><span className="text-[9px] text-slate-400 uppercase font-black tracking-widest mb-1">Tiempo</span><span className="text-lg font-bold text-slate-900">{service.duration_minutes}m</span></div>
              <div className="flex flex-col border-l border-slate-50 pl-4"><span className="text-[9px] text-slate-400 uppercase font-black tracking-widest mb-1">Buffer</span><span className="text-lg font-bold text-emerald-500">+{service.buffer_after_minutes}m</span></div>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="bg-white rounded-[56px] shadow-2xl p-12 max-w-xl w-full border border-slate-100 overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <select className="flex-1 bg-slate-50 border-slate-100 border-2 rounded-[20px] p-4 text-slate-900 font-bold outline-none" value={form.duration_hours} onChange={e => setForm({...form, duration_hours: parseInt(e.target.value)})}>{[0,1,2,3,4,5].map(h => <option key={h} value={h}>{h} h</option>)}</select>
                      <select className="flex-1 bg-slate-50 border-slate-100 border-2 rounded-[20px] p-4 text-slate-900 font-bold outline-none" value={form.duration_mins} onChange={e => setForm({...form, duration_mins: parseInt(e.target.value)})}>{[0,15,30,45].map(m => <option key={m} value={m}>{m} m</option>)}</select>
                    </div>
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <textarea className="w-full bg-slate-50 border border-slate-100 border-2 rounded-[24px] p-5 text-slate-900 font-medium h-32 outline-none focus:border-rose-400 resize-none" value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Describe el servicio..." />
              </div>
              <div className="flex gap-4 mt-10">
                ...
                <button onClick={handleSaveService} disabled={isLoadingAction || !form.service_name} className="flex-[2] bg-rose-600 text-white py-5 rounded-[24px] font-black shadow-xl hover:bg-rose-700 transition-all flex items-center justify-center gap-3"><Save size={20} /> {isLoadingAction ? 'Guardando...' : (editingServiceId ? 'Guardar Cambios' : 'Confirmar')}</button>
              </div>
            </div>
          </div>
        </div>
