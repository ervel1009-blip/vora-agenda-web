...
export default function PlanesPage() {
  ...
  useEffect(() => {
    const checkOnboardingIntegrity = async () => {
      ...
      // Validaciones de integridad en cascada
      ...
      if (!hCount || hCount === 0) { router.push('/dashboard/horarios'); return; }

      const { count: sCount } = await supabase.from('services_config').select('*', { count: 'exact', head: true }).eq('organization_id', org.id);
      if (!sCount || sCount === 0) { router.push('/dashboard/servicios'); return; }
